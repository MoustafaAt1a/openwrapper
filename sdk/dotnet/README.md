...
Production-ready .NET 8 client for the [OpenWrapper](https://github.com/openwrapper/openwrapper) payment gateway API.
...
Typed exceptions map to `error.code` from the gateway (`ValidationException`, `RateLimitException`, etc.). Transport failures throw `GatewayUnreachableException`.

```csharp
try
{
    await client.Payments.CreateAsync(params);
}
