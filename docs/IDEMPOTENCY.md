...
semantics (§11). This document defines each; `apps/gateway/src/store/` and the
provider adapters implement them.

## Boundary 1: client → OpenWrapper

- **Identity**: the client-supplied `Idempotency-Key` HTTP header
  (`crates/core/src/ids.rs::IdempotencyKey` — validated: 1–200 bytes, printable
...
- **Concurrency behavior**: the SQL `UNIQUE` constraint on
  ...
  `apps/gateway/src/store/sqlite.rs::tests::concurrent_identical_requests_only_one_proceeds`
...
- **Identity**: for Paymob, no explicit idempotency key is sent to the
  ...
  this endpoint (see `docs/research/paymob.md`); OpenWrapper's own boundary-1
  ...
  `crates/providers/fawry/src/client.rs::derive_merchant_ref_num`) is Fawry's own
...
- **Identity**: a provider-scoped `event_id` each adapter derives from
  ...
  - Paymob: `"paymob:{transaction_id}"` (`crates/providers/paymob/src/webhook.rs`).
  ...
  - Fawry: `"fawry:{requestId}"`, using the notification's own
    "UUID generated Request id" field (`crates/providers/fawry/src/webhook.rs`)
...
- **Duplicate behavior**: see above — idempotent no-op, tested in
  `apps/gateway/src/store/sqlite.rs::tests::webhook_event_dedup_only_admits_first_delivery`.
- **Payload mismatch / consistency behavior**: before any transition is
  ...
  `apps/gateway/src/store/sqlite.rs::tests::webhook_transition_is_rejected_when_amount_does_not_match_stored_payment`.
- **Failure behavior**: an illegal state transition (e.g. a webhook
  ...
  `apps/gateway/src/store/sqlite.rs::tests::illegal_transition_is_reported_not_silently_applied`.
